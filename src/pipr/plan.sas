...
%macro _pipe_plan_serialize(out_plan=);
  %local _plan;
  %let _plan=data=%superq(_pipe_plan_data)||keep=%superq(_pipe_plan_keep)||drop=%superq(_pipe_plan_drop)||rename=%superq(_pipe_plan_rename)||where=%superq(_pipe_plan_where)||stmt=%superq(_pipe_plan_stmt)||supported=%superq(_pipe_plan_supported)||unsupported=%superq(_pipe_plan_unsupported_steps);
  %_pipr_ucl_assign(out_text=%superq(out_plan), value=%superq(_plan));
%mend;

%macro _pipe_plan_deserialize(plan=);
  %local _n _i _seg _k _v _eq;
  ...
  %do _i=1 %to &_n;
    ...
    %if &_eq > 0 %then %do;
      ...
      %if &_k=DATA %then %let _pipe_plan_data=%superq(_v);
      %else %if &_k=KEEP %then %let _pipe_plan_keep=%superq(_v);
      %else %if &_k=DROP %then %let _pipe_plan_drop=%superq(_v);
      %else %if &_k=RENAME %then %let _pipe_plan_rename=%superq(_v);
      %else %if &_k=WHERE %then %let _pipe_plan_where=%superq(_v);
      %else %if &_k=STMT %then %let _pipe_plan_stmt=%superq(_v);
      %else %if &_k=SUPPORTED %then %let _pipe_plan_supported=%superq(_v);
      %else %if &_k=UNSUPPORTED %then %let _pipe_plan_unsupported_steps=%superq(_v);
    %end;
  %end;
%mend;
