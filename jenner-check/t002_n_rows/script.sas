...
/* Caller: exercise %n_rows from src/n_rows.sas against small mock datasets */
data work.policies;
    input policy_id premium;
    datalines;
1001 1200
1002 1800
1003 950
...
data work.empty;
    ...
    stop;
run;
%test_suite(sassyverse n_rows compatibility check);
    %test_case(row counts);
        %assertEqual(%n_rows(work.policies), 4);
        %assertEqual(%n_rows(policies), 4);
        %assertEqual(%n_rows(work.high), 2);
        %assertEqual(%n_rows(work.empty), 0);
    %test_summary;
%test_summary;
