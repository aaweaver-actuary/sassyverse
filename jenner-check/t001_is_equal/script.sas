...
%run_is_equal_tests;
/* Caller: exercise is_equal / is_not_equal from src/is_equal.sas */
%test_suite(sassyverse is_equal compatibility check);
    %test_case(numeric equality);
        %assertEqual(%is_equal(10, 10), 1);
        %assertEqual(%is_equal(10.0, 10), 1);
        %assertEqual(%is_equal(10, 11), 0);
    %test_summary;
    %test_case(character equality);
        %assertEqual(%is_equal(abc, abc), 1);
        %assertEqual(%is_equal(abc, abcd), 0);
        %assertEqual(%is_not_equal(abc, abcd), 1);
    %test_summary;
    %test_case(mixed types compare unequal);
        %assertEqual(%is_equal(10, abc), 0);
        %assertEqual(%is_equal(abc, 10), 0);
    %test_summary;
%test_summary;
