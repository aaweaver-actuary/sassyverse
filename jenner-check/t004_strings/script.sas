...
%run_string_tests;
/* Caller: exercise the string macros from src/strings.sas */
%test_suite(sassyverse strings compatibility check);
    %test_case(index find replace);
        %assertEqual(%str__index(andy is a cool guy, cool), 11);
        %assertEqual(%str__find(andy is a cool guy, cool), 11);
        %assertEqual(%str__index(andy is a cool guy, awesome), -1);
        %assertEqual(%str__replace(andy is a cool guy, cool, awesome), andy is a awesome guy);
    %test_summary;
    %test_case(len trim case);
        %assertEqual(%str__len(andy), 4);
        %assertEqual(%str__length(andy weaver), 11);
        %assertEqual(%str__trim( andy is a cool guy ), andy is a cool guy);
        %assertEqual(%str__upper(andy is a cool guy), ANDY IS A COOL GUY);
        %assertEqual(%str__lower(ANDY IS A COOL GUY), andy is a cool guy);
    %test_summary;
    %test_case(contains slice startswith endswith);
        %assertTrue(%str__contains(andy is a cool guy, cool), contains cool);
        %assertFalse(%str__contains(andy is a cool guy, awesome), does not contain awesome);
        %assertEqual(%str__slice(andy is a cool guy, 1, 4), andy);
        %assertEqual(%str__slice(andy is a cool guy, 11, 14), cool);
        %assertTrue(%str__startswith(andy is a cool guy, andy), starts with andy);
        %assertTrue(%str__endswith(andy is a cool guy, guy), ends with guy);
    %test_summary;
%test_summary;
